...
$(warning $(FLAGS))
VPATH+=$(dir $(CLASSDESC))/../include $(dir $(CLASSDESC))
endif

ifdef AEGIS
FLAGS+=-Werror -Wall -Wno-unused-local-typedefs -Wno-unused-variable -Wno-unused-function -Wno-unknown-pragmas -Wno-unknown-warning-option
ifeq ($(CPLUSPLUS),g++)
FLAGS+=-Wno-restrict
endif
endif
...
BASIC_CLEAN=rm -rf *.o *~ "\#*\#" core *.d *.cd *.gcda *.gcno
